...
/** Client heartbeat cadence — locked (ARCH §9.14), same as the generation socket. */
const CLIENT_HEARTBEAT_MS = 25_000;
/** MediaRecorder chunk interval — small enough to keep turn latency low. */
...
/**
 * "Talk to AI" voice conversation during lecture creation (T-121, #25).
 *
 * Simpler lifecycle than {@link ../ws/lecture-generation-socket}'s stream: a
 * dropped voice call just ends (per flow-5 §5.4, closing terminates the
 * session server-side too) — the teacher re-opens rather than auto-resuming
 * mid-conversation, so there's no reconnect/backoff here by design.
 */
export function useLectureVoiceSession(
  ...
  const startSession = useCallback(() => {
    ...
    const ws = new WebSocket(
      `${base}/lectures/${lectureId}/voice?language=${encodeURIComponent(language)}`
    );
