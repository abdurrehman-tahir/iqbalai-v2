...
export function TeachersClient() {
  const t = useTranslations("school_admin.teachers");
            ...
            <tbody className="divide-y divide-gray-100">
              {data.map((teacher) => (
                <tr key={teacher.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-600">{teacher.status}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {inviteSuccess ? (
          ...
          <form onSubmit={inviteForm.handleSubmit(handleInviteSubmit)} className="space-y-4">
            <div>
              ...
              <Input id="teacher-email" type="email" {...inviteForm.register("email")} />
            </div>
            <div>
              <Label htmlFor="teacher-name" required>
                {t("invite_modal.name_label")}
              </Label>
              <Input id="teacher-name" {...inviteForm.register("display_name")} />
            </div>
