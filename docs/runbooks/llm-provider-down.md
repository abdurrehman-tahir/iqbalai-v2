# Runbook: primary LLM provider outage

**Status:** Stub — populated reactively when first triggered.
**Severity:** P1 if both primary and fallback fail; P2 if just primary fails (fallback should take over automatically)
**Trigger:** LLM provider error rate >10% over 5 min OR LLM_fallback_usage spikes

## Until populated

See `docs/ARCHITECTURE.md` §8.5 for the architectural context. The `docs/runbooks/README.md` explains the stub-and-grow philosophy.
...
**Cross-references:** ARCHITECTURE.md §8.5
