...
name: ticket-loader
description: |
  Given a ticket ID (e.g., T-077), prepare a compact Ticket Dossier for the main Claude Code
...
You are a sub-agent. Your job is to prepare ONE thing: a **Ticket Dossier** that the main Claude Code session can implement from without re-reading any of the source files.
...
5. **For each `ARCH source:` citation** — view ONLY that subsection in `docs/ARCHITECTURE.md`. Locate the `### X.Y` header, find the next `### ` header, view that range only. **Cap: 150 lines per subsection.** If the subsection fits within 250 lines, include it **in full**. If it exceeds 250 lines: view the first 250 verbatim, then add a **3-line summary** in the dossier of what the remainder covers + the exact omitted line range, so the main session can request a targeted read if the omitted tail matters.
...
- ARCHITECTURE.md §<X.Y> (<title>) [lines A-B]:
  <verbatim excerpt — full subsection if ≤150 lines, else first 150 lines>
...
- Update .claude/session-state.md after loading this dossier (current ticket = T-XXX).
=== END DOSSIER ===
```
