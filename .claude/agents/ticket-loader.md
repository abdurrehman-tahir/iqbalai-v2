...
5. **For each `ARCH source:` citation** — view ONLY that subsection in `docs/ARCHITECTURE.md`. Locate the `### X.Y` header, find the next `### ` header, view that range only. **Cap: 150 lines per subsection.** If the subsection fits within 150 lines, include it **in full**. If it exceeds 150 lines: view the first 150 verbatim, then add a **3-line summary** in the dossier of what the remainder covers + the exact omitted line range, so the main session can request a targeted read if the omitted tail matters.
