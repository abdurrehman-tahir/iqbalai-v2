"""Teaching Innovation Record API — independent tenant (T-138, Flow 5 §3.10 #36).

Mirrors router.py — simplified own-memory variant, no cross-tenant influence.
"""
...
router = APIRouter(prefix="/independent/teachers/me/coaching", tags=["teacher-coaching"])


@router.get(
    "",
    response_model=SuccessEnvelope[list[CoachingSuggestionRead]],
    operation_id="independent_teacher_list_coaching_suggestions",
...
async def list_coaching_suggestions(
    ...
    teacher_id = await service.resolve_independent_teacher_id(db, claims)
    rows = await service.list_current_suggestions_independent(db, teacher_id)
...
@router.post(
    ...
    operation_id="independent_teacher_respond_to_coaching_suggestion",
...
async def respond_to_coaching_suggestion(
    ...
    teacher_id = await service.resolve_independent_teacher_id(db, claims)
    memory = await service.respond_to_suggestion_independent(
