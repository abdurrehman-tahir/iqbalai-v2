"""Voice conversation session service — T-121 (Flow 5 §3.4 / #25).

Open/turn/close lifecycle: STT (faster-whisper) -> LLM edit interpretation
-> structured draft edit (insert/replace/append, applied as a new immutable
lecture version, §4.18) -> TTS confirmation. Single active session per
teacher per lecture (§5.4 edge case). Raw audio uploaded to the `audio`
MinIO bucket; ``tasks.py``'s Celery beat purges it after
``VOICE_AUDIO_RETENTION_HOURS`` (transcripts are kept indefinitely).
"""
...
from app.features.lectures.repository import (
    ...
    LectureVoiceSessionRepository,
    LectureVoiceTurnRepository,
)
...
class VoiceSessionConflictError(Exception):
    """Another active session already exists for this teacher+lecture (flow-5 §5.4:
    "Second tab gets 'voice session in progress in another tab' warning.")."""
...
async def open_session(
    session: AsyncSession, *, lecture: SchoolLecture, teacher_user_id: str
) -> SchoolLectureVoiceSession:
    repo = LectureVoiceSessionRepository(session)
    ...
    voice_session = SchoolLectureVoiceSession(
        lecture_id=lecture.id, teacher_user_id=teacher_user_id
    )
    logger.info("voice_session_opened", lecture_id=lecture.id, teacher_user_id=teacher_user_id)
    return await repo.create(voice_session)


async def close_session(
    session: AsyncSession, voice_session: SchoolLectureVoiceSession
) -> SchoolLectureVoiceSession:
    logger.info(
        "voice_session_closed", session_id=voice_session.id, lecture_id=voice_session.lecture_id
    )
    return await LectureVoiceSessionRepository(session).end(voice_session)


async def _apply_edit_operation(
    session: AsyncSession, *, lecture: SchoolLecture, operation: VoiceEditOperation
) -> SchoolLectureVersion | None:
    """Applies insert/replace/append as a NEW immutable version (§4.18). ``None`` op -> no-op."""
    ...
    paragraphs = await LectureParagraphRepository(session).list_by_version(
    ...
    current_version = await session.get(SchoolLectureVersion, lecture.current_version_id)
    next_version_num = (current_version.version + 1) if current_version is not None else 1
    new_version = SchoolLectureVersion(
...
def _parse_voice_edit_output(raw: str, transcript: str) -> VoiceEditOutput:
    ...
    try:
        ...
        # Honest fallback: no draft change, echo the transcript per §5.4's
        # "Did you mean: ..." rule so the teacher can restate the instruction.
        logger.warning("voice_edit_parse_failed", prompt_version=PROMPT_VERSION)
...
async def process_turn(
    ...
    *,
    ...
    audio_bytes: bytes,
    target_language: str,
) -> tuple[SchoolLectureVoiceTurn, bytes | None, str | None]:
    """One STT -> LLM -> draft-edit -> TTS round trip (target STS loop < 2s, §6 Limits).

    Returns ``(turn, tts_audio_or_None, unavailable_notice_or_None)`` — per the
    locked fallback (§5.4), a missing TTS voice never raises; it degrades to a
    text notice the caller relays instead of audio.
    """
    ...
    turns_repo = LectureVoiceTurnRepository(session)
    ...
    try:
        ...
        logger.info("voice_tts_unavailable", language=exc.language, reason=exc.reason)
