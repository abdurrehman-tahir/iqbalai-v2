"""Voice conversation WebSocket — T-121 (Flow 5 §3.4 / #25).

Mounted at `/ws/v1/lectures/{lecture_id}/voice`. Unlike the generation WS
(T-117, a live token relay), this is request/response per utterance: the
client streams base64 audio chunks then a turn-end marker; the server runs
STT -> LLM edit interpretation -> draft update -> TTS and replies with the
transcript, confirmation, and (when available) synthesized audio. Same A-003
cookie auth + ownership shape as the generation WS.
"""
...
from app.features.lectures.models import SchoolLecture, SchoolLectureVoiceSession
...
router = APIRouter(prefix="/lectures", tags=["lecture-voice-ws"])

# Safety cap per turn's buffered audio — a runaway client must not exhaust memory.
...
async def _process_and_respond(
    ...
    *,
    ...
    try:
        ...
        logger.error(
            "lecture_voice_turn_failed", lecture_id=lecture_id, user_id=user_id, error=str(exc)
        )
...
async def lecture_voice_ws(
    ...
    try:
        ...
        # 4409 (application range) mirrors HTTP 409 Conflict — flow-5 §5.4's
        # "voice session in progress in another tab" warning.
    ...
    try:
        ...
        while True:
            ...
            if msg_type == "voice_turn_end":
                ...
                audio_buffer = bytearray()
                continue
            # Unknown types ignored — forward-compatible per §5.12's domain-event convention.

        await _close_quietly(websocket, 1000)
    except WebSocketDisconnect:
        logger.info("lecture_voice_ws_disconnected", lecture_id=lecture_id, user_id=user_id)
    except TimeoutError:
        logger.info("lecture_voice_ws_heartbeat_timeout", lecture_id=lecture_id, user_id=user_id)
