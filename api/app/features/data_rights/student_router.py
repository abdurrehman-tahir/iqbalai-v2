"""Student data rights endpoints — T-084."""
...
router = APIRouter(prefix="/students/me/data-rights", tags=["data-rights"])


@router.get(
    "",
    response_model=SuccessEnvelope[DataRightsStatusRead],
    operation_id="student_get_data_rights",
    summary="Get student data-rights status",
...
async def get_data_rights(
    ...
    status = await svc.get_status(claims, role=UserRole.STUDENT)
...
@router.post(
    ...
    operation_id="student_request_data_export",
    status_code=201,
...
@router.get(
    ...
    response_model=None,  # binary file download — no Pydantic response model
    operation_id="student_download_data_export",
...
@router.post(
    "/deletion",
    response_model=SuccessEnvelope[DataRightsRequestRead],
    operation_id="student_request_account_deletion",
    status_code=201,
...
@router.post(
    ...
    response_model=SuccessEnvelope[DataRightsRequestRead],
    operation_id="student_cancel_account_deletion",
