"""School migration: teacher_ai_memory table (T-129, Flow 5 §3.10 #36).

Revision ID: school_0060
Revises: school_0059
Create Date: 2026-09-11

Purpose: per-teacher adaptive coaching memory (Teaching Innovation Record,
T-138). ``category`` is a plain string column, not a Postgres enum, so future
milestones can add categories without a migration; ``teacher_response`` is a
stable 3-value enum.
...
revision: str = "school_0060"
down_revision: str = "school_0059"
branch_labels: tuple[()] = ()
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("tenant_type", sa.String(20), nullable=False, server_default="school"),
        sa.Column(
            "teacher_user_id",
            sa.String(36),
