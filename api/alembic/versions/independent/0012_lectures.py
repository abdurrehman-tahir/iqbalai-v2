"""Independent migration: lecture tables (T-113).

Revision ID: independent_0012
Revises: independent_0011
Create Date: 2026-07-30

Purpose: lectures / lecture_versions / lecture_drafts / lecture_paragraphs (independent).
Risk: low — additive; school_id + offering_id constrained NULL (no cross-schema FKs).
Reversible: yes
"""
...
revision: str = "independent_0012"
down_revision: str = "independent_0011"
branch_labels: tuple[()] = ()
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # Shape parity with school — always NULL; CHECK enforces (no school FKs).
        sa.Column("school_id", sa.String(36), nullable=True),
        sa.Column("grade_subject_offering_id", sa.String(36), nullable=True),
        sa.Column(
            "teacher_user_id",
            sa.String(36),
        ...
        sa.CheckConstraint("school_id IS NULL", name="lectures_independent_school_null_check"),
        sa.CheckConstraint(
