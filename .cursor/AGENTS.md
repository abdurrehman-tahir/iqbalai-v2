...
**You are working on IqbalAI v2 in Cursor.** This file is the Cursor agent's project instructions. Claude Code users follow the parallel rules in `.claude/CLAUDE.md` — both toolchains coexist; do not remove or merge them.
...
Four skills are installed at `.cursor/skills/`. **Skills are heavy — read each SKILL.md ONLY when its narrow trigger truly fires.** Cursor rules in `.cursor/rules/skill-*.mdc` enforce the same triggers as this table. Pre-commit hooks + CI catch any forbidden imports / stack violations regardless, so a missed trigger costs a re-commit, not a shipped bug.
...
**Skill priority over body rules:** when the body of this AGENTS.md and a skill's SKILL.md both speak to the same concern, the skill's guidance is more specific and wins. The body rules below remain in force as the default; the skills add depth.

## Sub-agents — your first action on any ticket

Cursor can launch **subagents** (Task tool) that run in their own context window and return only a compact result to this main session. Use them to keep the main context lean. Agent definitions live in `.cursor/agents/`; enforcement rule: `.cursor/rules/ticket-loader.mdc`.

**MANDATORY first action when implementing any ticket:** when Hamza says any variant of `"Implement T-XXX"` (with or without a milestone path), your FIRST action — before any file read, any plan, any code — is to launch a **Task subagent** (`readonly: true`) following `.cursor/agents/ticket-loader.md`:

> Launch a readonly Task subagent per `.cursor/agents/ticket-loader.md` to prepare ticket T-XXX.

The subagent locates the ticket, reads the cited spec § + cited ARCH §§ in its own context, and returns a compact **Ticket Dossier** (ticket body, rule excerpts, ARCH summaries, deps, acceptance criteria) to you. From that point you work from the dossier. **Reading the full milestone file, the full flow spec, or wide ranges of ARCHITECTURE.md directly in this main session is forbidden** — the only exception is following a specific cross-reference the dossier explicitly flagged. If the dossier is malformed or missing a field, abort and tell Hamza; do not paper over it by reading the source files yourself.
...
**Prefer subagents for parallel exploration.** If a task has multiple independent searches, audits, or extractions to do (e.g. "find every place a deprecated API is used," "verify acceptance items 1-8 in parallel"), launch subagents instead of doing them serially in the main session. Each subagent runs in its own context and returns only the result.

## Session-state recovery (compaction)

This main session may be compacted mid-work. To recover cleanly without re-reading everything, maintain `.cursor/session-state.md` — a short file (≤30 lines) you update after each meaningful step (ticket loaded, file edited, tests written, commit made). Format: current ticket ID, status, last file edited, next intended step, dossier-source list. After a compaction, read this file first; it replaces the need to re-fetch the milestone/spec/ARCH from scratch.

## What to read — and when (conditional, not pre-loaded)

The ONLY files you load at session start are this AGENTS.md and `docs/ARCHITECTURE.md` §0 (the task→sections quick-index). Every other reference doc is loaded **conditionally**, the moment its concern triggers — never pre-emptively. This keeps the main context lean and is essential for fitting a milestone's worth of tickets in one Cursor session.
...
| `docs/WORKFLOW.md` | Hamza's very first day, OR you can't recall a specific process step. The day-to-day loop is summarized in this AGENTS.md ("Workflow you must follow" below). Don't reload WORKFLOW per ticket. |
...
| `docs/backlog/M-NN-*.md` | The `ticket-loader` subagent reads this for you. Reading it directly in the main session is forbidden (see "Sub-agents" above). |
...
2. **Your literal first action: invoke the `ticket-loader` subagent** with the ticket ID. The subagent returns a compact Ticket Dossier (ticket body, cited spec excerpts, ARCH summaries, deps, acceptance). Do NOT read the milestone / spec / ARCH directly in this main session.
3. **Verify deps:** the dossier lists `Depends on:` — confirm those tickets are marked done before proceeding.
4. **Read narrow skills only if their trigger fires** (see skill table above). Update `.cursor/session-state.md` with the ticket ID + intended next step.
...
8. **Close out the ticket (mandatory, every ticket — not just at milestone close):** in the milestone file, set that ticket's `Status: done` and record the commit SHA on the ticket. Then update `.cursor/session-state.md` (done list + next ticket). The milestone file is the durable per-ticket ledger; `session-state.md` is the fast within-session hint.
...
If Hamza asks you to "just build milestone X" without picking a specific ticket, **start with T-NNN at the top of the milestone file**. Before that first ticket, run the **ticket-modernization check**: if the milestone's tickets predate the current template (no API-contract / Tests / UX-acceptance fields, or data-model shown as raw DDL), the global rules still apply in full — data-model = intent (model-first), and the Tests + UX-acceptance + generated-typed-client gates hold regardless of the ticket's wording. If he asks to skip the backlog and "just implement Flow 5" or "just build Phase 1," **refuse** and refer him to WORKFLOW.md.
...
7. **Frontend + contract + type gates exist and block.** CI MUST run `pnpm test` (Vitest + RTL) and a **Playwright smoke** on every PR (full Playwright suite nightly); a **typed-client drift check** that regenerates `schema.d.ts` from the live OpenAPI and fails if it differs from the committed file; and a **`response_model` presence check** across all routes. A green PR with zero frontend tests, a drifted client, or a route missing `response_model` means this invariant was violated. Additionally: the create/update `@smoke` paths MUST run against the **real composed backend** (not mocked), a **contract check** MUST assert mock + client request shapes match the generated `*Create`/`*Update` schemas, and a **`client-request-type` lint** MUST reject inline object-literal request types in `api.ts`/`index.ts`. A green PR where every create 422s against the live API means these gates are missing — `typed-client-drift` proves only that the types are *fresh*, not that the client *uses* them (M-01a).
...
If you generate code that fails these hooks, the user will see the failure. Fix it and re-commit. Don't ask the user to disable hooks.

## Cursor toolchain files (parity with `.claude/`)

| Cursor | Claude Code | Purpose |
|---|---|---|
| `.cursor/AGENTS.md` | `.claude/CLAUDE.md` | Main project rules |
| `.cursor/rules/*.mdc` | (built into CLAUDE.md) | Auto-loaded rule triggers |
| `.cursor/skills/` | `.claude/skills/` | Domain skills |
| `.cursor/agents/ticket-loader.md` | `.claude/agents/ticket-loader.md` | Ticket dossier subagent |
| `.cursor/session-state.md` | `.claude/session-state.md` | Session recovery scratchpad |
| `.cursor/permissions.json` | `.claude/settings.local.json` | Terminal/MCP command allowlist |
| `.cursor/cli.json` | — | Cursor CLI agent permissions |

**Allowlist note:** `.cursor/permissions.json` replaces the in-app terminal allowlist when Run Mode (Allowlist or Auto-review) is enabled in Cursor Settings. Match Claude's pre-approved commands without repeated approval prompts.
...
- You'd be touching more than 5 files in a single change AND the changes are not a single coherent vertical slice already authorised by the ticket.
...
At session start, this AGENTS.md loads and you read `docs/ARCHITECTURE.md` §0. **Do NOT print a confirmation message** listing what you read. **Do NOT pre-load** STACK_LOCK / WORKFLOW / DEVIATIONS / AMENDMENTS / BRANCHING / ENV_VARS — load each only when its trigger fires (table above).

**Resume rule (where to start, even if Hamza says nothing specific):** before waiting, determine the resume point — (1) read `ROADMAP.md` to find the currently-active milestone (the first not `done`); (2) read that milestone file's ticket `Status:` fields — **the first ticket not marked `done` is where you resume**; (3) `.cursor/session-state.md` is a fast hint for the same thing. **If `session-state.md` and the milestone file disagree, the milestone file's `Status:` fields win** (they're the durable source of truth; session-state is only a scratchpad). If Hamza names a specific ticket, that overrides the resume point. Then your literal first action for that ticket is to invoke the `ticket-loader` subagent.

After a context compaction, read `.cursor/session-state.md` first to recover (do NOT re-fetch the milestone/spec/ARCH from scratch).

---

**Last updated:** 2026-06-24
**Owner:** @abdurrehman-tahir
**Toolchain:** Cursor (parallel rules for Claude Code live in `.claude/CLAUDE.md`)
